...
/**
 * Класс телефонная книга. Приложение создано с использованием структуры данных Set (альтернатива Map)
 * Set<Contact> contacts - коллекция для хранения экземпляров класса Contact.
...
public class PhoneBook {
     ...
     *
     * @param number     - номер телефона. Не может быть null, пустым
     * @param surname    - фамилия. Не может быть null, пустым
     * @param name       - имя. Не может быть null, пустым
     * @param patronymic - отчество. Не может быть null, пустым
     */
    public void add(String number, String surname, String name, String patronymic) {
        if (isTextInappropriate(number) || isTextInappropriate(surname) ||
                isTextInappropriate(name) || patronymic == null) {
            System.out.println("Номер телефона,фамилия и имя должны не быть null или пустыми, " +
                    "отчество не должно быть null");
            return;
        }
        ...
        boolean isRecordUnique = contacts.add(new Contact
                (number, surname.trim(), name.trim(), patronymic.trim())); //при успешном добавлении возвращается true

        if (isRecordUnique) {
            System.out.println("Номер телефона: " + number +
                    " успешно внесен в телефонную книгу");
        } else {
            String previousAddition = contacts.stream()
    ...
    public void find(String surname) {
        ...
        listOfNumbers = contacts.stream()
                .filter(el -> el.getSurname().equals(inputSurname))
    ...
    public void containsPhoneNumber(String number) {
        ...
        System.out.printf("Телефонная книга%s содержит телефонный номер: %s\n",
                (contacts.stream().anyMatch(el -> el.getNumber().equals(number)) ? "" : " не"),
                number);
    }
